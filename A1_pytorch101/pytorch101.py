...
def count_tensor_elements(x: Tensor) -> int:
    ...
    for i in range(len(x_size)) :
        num_elements = num_elements * x_size[i]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    elif start + (10-start%10) <= stop :
        x = torch.arange(start + (10-start%10), stop, 10)
    else :
        x = torch.tensor([])
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    x_3d = x.view(2, 3, 4)
    y = x_3d.transpose(1, 2).contiguous().view(8, 3).t()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    row_min_val, row_min_idx = x.min(dim=1)
    idx = torch.arange(len(row_min_val))
    y = x[:, :].clone()
    y[idx, row_min_idx] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    y = x.clone()
    u = y.sum(0) / y.shape[0]
    std = torch.sqrt(((y-u)**2).sum(0) / (y.shape[0]-1))
    y = (y-u) / std
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
